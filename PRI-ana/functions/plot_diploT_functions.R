...
fcs$dodiploT <- function(mode, totalID=NA, total.cex=NA) {
  ...
  for (i in 1:(len - 1)) {
    ...
    if (mode == "single") {
      ...
      if (!is.na(totalID)) {
        this$bindiplot(data[, c(colvec[i], var1.num)], cutoffs[c(i, len)], bin.pos = pos.y, totalcex = total.cex)
      } else {
        this$bindiplot(data[, c(colvec[i], var1.num)], cutoffs[c(i, len)], bin.pos = pos.y)
      }
    }
  }
...
fcs$newdiploT <- function(len=26) {
  ...
  if (checkDATE == "1") {
    date <- gsub("-", "", Sys.Date())
    title(main=date, outer= TRUE, line=1, cex.main=1.0, adj=1)
  }
...
fcs$dodiGraph <- function(mode, total.cex=NA, set.zero=FALSE) {
  ...
  if (grepl("^\\(", xmin)) {
    xmin <- as.numeric(strsplit(xmin, "\\(")[[1]][2])
  } else {
    xmin <- as.numeric(strsplit(xmin, "\\[")[[1]][2])
  }
  xmin <- xmin - 2 * binSize
...
fcs$bindiplot <- function(
  ...
  if (FALSE) {
    ...
    bin.pos <- this$distepy
    set.cex <- 1.1
    set.cex.axes <- totalcex <- 1.0
  }
  ...
  if (!file.exists(metadatafile)) {
    ...
    write.table(t(header), metadatafile, sep = "\t",
  ...
  if (nrow(data) == 0) {
    ...
    if (checkCALC == "freq") {
      ...
      if (is.infinite(min.y)) {
        ...
        if (checkCALC == "density" & max.y < 200) col.minmax <- "red"
      ...
      if (!empty) {
        ...
        ### bin color factor
        my.calc.fac <- cut(my.calc$x, breaks=steps, labels=1:10, include.lowest=TRUE)
        # levels(my.calc.fac) <- c(0, levels(my.calc.fac), 11, 12)
        ### if x < min.range
        my.calc.fac[which(my.calc$x < steps[1] & my.calc$ncells >= mincells)] <- 1
        ### if x > max.range
        my.calc.fac[which(my.calc$x > steps[11] & my.calc$ncells >= mincells)] <- 10
      }
    }
    ...
    if (!empty) {
      ...
      for (i in min(idx, idx.lower):max(idx, idx.lower)) {
        ...
        if (grepl("^\\(", sub1)) {
          ### if it starts with a "("
          sub2 <- as.numeric(strsplit(sub1, "\\(")[[1]][2])
        } else {
          ### if it starts with a "["
          sub2 <- as.numeric(strsplit(sub1, "\\[")[[1]][2])
        }
      ...
      if (cutoffs[2] != 0 & checkCALC == "freq") {
        ...
        if (cutoffs[1] != 0) {
          ...
          csv.content <- cbind(current.date, displayfile,
            this$current.cofactor, checkCALC,
            colnames(data)[2], colnames(data)[1],
            minVal.B, maxVal.B, absRange,
            cutoffs[1], cutoffs[2],
      ...
      printf("w: diploT meta1 written in %s.", metadatafile)
      
      ### print min/max value
      if (any(checkCALC == c("density", "freq", "RSEM"))) {
        print.minmax <- sprintf("min/max=%0.0f/%0.0f", minX, maxX)
      } else {
